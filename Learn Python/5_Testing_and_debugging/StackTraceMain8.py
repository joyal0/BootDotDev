def create_stats_message(strength, wisdom, dexterity):
      ...
      msg = f"You have {strength} strength, {wisdom} wisdom, \
and {dexterity} dexterity for a total of {total} stats."
      return msg


"""
Stack Trace
A stack trace (or "traceback") is a scary-looking 
error message that the Python interpreter prints 
to the console when it encounters certain problems. 
Stack traces are most common (at least for you right 
now) when you're trying to run invalid Python code.

You need to get used to figuring out scary error messages 
as a programmer. We might as well start now.

Assignment
Go ahead and run the code in its current state. 
...
This is a standard header that's just letting us know 
that a Python traceback is what we're looking at.

2.  File "<exec>", line 6, in <module> and File "<string>", line 1, in <module>
This is the start of the "trace". These strange 
"<exec>" and "<string>" files don't really exist, 
the Python interpreter is letting us know about them 
because they have to do with how your code is executed 
in a virtual browser-based environment.

3.  File "/home/pyodide/main.py", line 3
Now we're getting to the real meat of the error message! 
The purpose of a "trace" is to show us the path that the
 Python interpreter took through our code before it 
 encountered the error, which can help us figure out what went wrong.
In this case, the interpreter was executing the code in
 the main.py file, and it got to line 3 before it encountered 
 the error.
